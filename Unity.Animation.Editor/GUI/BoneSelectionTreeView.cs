...
{
    internal class BoneSelectionTreeView : TreeView
    {
        public BoneSelectionTreeView(TreeViewState state)
            : base(state)
        {
        ...
        protected override void SelectionChanged(IList<int> selectedIds)
        {
            ...
            if (Skeleton == null)
                return;
            if (selectedIds.Count == 0 || Skeleton.ActiveTransformChannels == null || selectedIds[0] < 0 || selectedIds[0] >= Skeleton.ActiveTransformChannels.Count)
                SelectionHasChanged?.Invoke(new SkeletonBoneReference(Skeleton, TransformBindingID.Root));
            else
                SelectionHasChanged?.Invoke(new SkeletonBoneReference(Skeleton, Skeleton.ActiveTransformChannels[selectedIds[0]].ID));
        }
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            if (Skeleton != null)
            {
                ...
                for (int i = 0; i < Skeleton.ActiveTransformChannels.Count; ++i)
                {
                    var treeViewItem = new TreeViewItem(i, -1, Skeleton.ActiveTransformChannels[i].ID.Name);
                    s_TreeViewItems.Add(treeViewItem);
                    s_TransformBindingIDToTreeView[Skeleton.ActiveTransformChannels[i].ID] = treeViewItem;
                }

                s_TreeViewItems.Sort();

                var rootBoneID = TransformBindingID.Root;
                // Build parent bones.
                for (int i = 0; i < s_TreeViewItems.Count; ++i)
                {
                    var channelIndex = s_TreeViewItems[i].id;
                    var ID = Skeleton.ActiveTransformChannels[channelIndex].ID;
